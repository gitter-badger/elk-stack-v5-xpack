...
'
```
* logstash default, with raw fields

```bash
curl -XDELETE http://elastic:changeme@localhost:9200/_template/logstash\?pretty
curl -XDELETE 'http://elastic:changeme@localhost:9200/logstash*'
curl -XPUT 'http://elastic:changeme@localhost:9200/_template/logstash' -d '{
  "template" : "logstash-*",
  "settings" : {
